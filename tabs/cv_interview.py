...
from utils.chat_sessions import create_chat_session, get_user_sessions
...
def cv_interview_tab():
    ...
    if "current_session_id" not in st.session_state:
        ...
        sessions = get_user_sessions(user_id, tab_name)
        if sessions:
            # Load most recent session
            st.session_state.current_session_id = sessions[0][0]
            from utils.chat_sessions import get_session_messages
            messages = get_session_messages(sessions[0][0])
            ...
            st.session_state.current_session_id = create_chat_session(user_id, tab_name)
            st.session_state.chat_messages = []
            
    show_chat_library(user_id)
...
JOB DESCRIPTION: {job_description if job_description else 'Not provided'}"""
                ...
                save_chat_message(user_id, session_id, tab_name, "assistant", response)
                st.rerun()
...
# with chat_col:
#     if st.button("📜 View Chat History", key="cv_chat_history"):
